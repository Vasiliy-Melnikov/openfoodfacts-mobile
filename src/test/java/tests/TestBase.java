...
public class TestBase {
    ...
    private boolean isBrowserStack() {
        ...
        String host = (hostRaw == null || hostRaw.isBlank())
                ? "local"
                : hostRaw.toLowerCase();
    ...
    void tearDown(TestInfo testInfo) {
        ...
        if (isBrowserStack() && !sessionId.isBlank()) {
            try {
    ...
    private void ensureAppReady(Duration timeout) {
        ...
        throw new AssertionError(
                "App not ready within " + timeout.toSeconds() + "s (Scan screen not shown)"
        );
    }

    private boolean tapAndroidPermissionAllowIfPresent() {
        try {
            var allow1 = io.appium.java_client.AppiumBy.id(
                    "com.android.permissioncontroller:id/permission_allow_button");
            var allow2 = io.appium.java_client.AppiumBy.id(
                    "com.android.permissioncontroller:id/permission_allow_foreground_only_button");
            var allow3 = io.appium.java_client.AppiumBy.id(
